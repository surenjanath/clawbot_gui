...
pub struct OpenAiCompatConfig {
    ...
    pub default_request_path: &'static str,
...
impl OpenAiCompatConfig {
    ...
    pub const fn ollama() -> Self {
        Self {
            ...
            // Ollama exposes OpenAI-compatible chat at /v1/chat/completions (native /api/chat uses a different schema).
            default_request_path: "v1/chat/completions",
...
impl OpenAiCompatClient {
    ...
    pub fn new(api_key: impl Into<String>, config: OpenAiCompatConfig) -> Self {
        Self {
            ...
            request_path: config.default_request_path.to_string(),
    ...
    pub async fn send_message(
        ...
        let body = response
            .text()
            .await
            .map_err(|err| ApiError::Http(err))?;
        let payload = serde_json::from_str::<ChatCompletionResponse>(&body).map_err(ApiError::from).map_err(|err| {
            eprintln!("DEBUG: OpenAiCompat send_message response body: {body}");
            err
        })?;
    ...
    async fn send_raw_request(
        ...
        let request_url = endpoint(&self.base_url, &self.request_path);
...
fn normalize_response(
    ...
    let (id, message, finish_reason) = if let Some(choice) = response.choices.into_iter().next() {
        (
            response.id,
            choice.message,
            choice.finish_reason.or(response.done_reason),
        )
    } else if let Some(message) = response.message {
        (response.id, message, response.done_reason)
    } else {
        return Err(ApiError::InvalidSseFrame(
            "chat completion response missing choices and message",
        ));
    };
...
fn endpoint(base_url: &str, request_path: &str) -> String {
...
mod tests {
    use super::{
        build_chat_completion_request, endpoint, normalize_finish_reason,
    ...
    fn endpoint_builder_accepts_base_urls_and_full_endpoints() {
        ...
        assert_eq!(
            endpoint("https://api.x.ai/v1/chat/completions", "chat/completions"),
            "https://api.x.ai/v1/chat/completions"
        );
    }
