...
#[test]
fn provider_client_routes_ollama_models_through_ollama() {
    let _lock = env_lock();
    let _ = EnvVarGuard::set("OLLAMA_BASE_URL", Some("https://example.ollama.test"));

    let client = ProviderClient::from_model("gemma3").expect("ollama model should resolve");
