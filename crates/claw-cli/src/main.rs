...
use api::{
    resolve_startup_auth_source, AuthSource, ClawApiClient, ProviderClient, ContentBlockDelta,
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Tui => run_tui()?,
        CliAction::Gui => gui::run_gui()?,
        CliAction::Repl {
            model,
            allowed_tools,
            permission_mode,
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            ...
            Err(error) => {
                ...
                eprintln!("DEBUG: request error full: {:#?}", error);
                Err(Box::new(error))
            }
        }
    }
        ...
        match handle_menu_input(&input) {
            TuiMode::Chat => {
                ...
                loop {
                    ...
                    io::stdin().read_line(&mut chat_input)?;
                    
                    if let Some(mode) = handle_chat_input(&chat_input) {
                        if matches!(mode, TuiMode::Exit) {
                            return Ok(());
                        }
                        break;
                    }
                    ...
                    if !chat_input.trim().is_empty() {
                        println!("[Chat input: {}]", chat_input.trim());
                    }
                }
            }
            TuiMode::Settings => {
                loop {
                    ...
                    if handle_settings_input(&input) == TuiMode::Chat {
                        break;
                    }
                }
            }
...
impl DefaultRuntimeClient {
    fn new(
        ...
        let client = match provider_kind {
            ...
            _ => ProviderClient::from_model_with_default_auth(&model, None)?,
        };
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        self.runtime.block_on(async {
            ...
            let mut stream = match self.client.stream_message(&message_request).await {
                ...
                Err(error) => {
                    eprintln!(
                        "WARN: stream_message failed; falling back to send_message: {error}"
                    );
                    let response = self
                        .client
            ...
            loop {
                let event = match stream.next_event().await {
                    ...
                    Err(error) => {
                        eprintln!(
                            "WARN: stream error; falling back to non-stream send_message: {error}"
                        );
                        let response = self
                            .client
                            .send_message(&MessageRequest {
